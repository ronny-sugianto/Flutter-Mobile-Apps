...
class VideoPlayerProgressSeekBarWidget extends LeafRenderObjectWidget {
  ...
  const VideoPlayerProgressSeekBarWidget({
    Key? key,
  ...
  /// wrap [VideoPlayerProgressSeekBarWidget] with a `Padding` widget if your layout needs to leave
...
class _RenderProgressBar extends RenderBox {
  ...
  TextPainter _leftTimeLabel() {
    final text = Utils().getVideoTimeString(progress);
  ...
  TextPainter _rightTimeLabel() {
    switch (timeLabelType) {
      case TimeLabelType.totalTime:
        final text = Utils().getVideoTimeString(total);
        return _layoutText(text);
      case TimeLabelType.remainingTime:
        final remaining = total - progress;
        final text = '-${Utils().getVideoTimeString(remaining)}';
        return _layoutText(text);
    }
  }
